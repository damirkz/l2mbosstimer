...
def init():
	...
	regenembed.set_footer(text = '--')#' : 멍 보스')
			...
			if timestring == bossTime[i]:
				if bossTimeString[i] != '99:99:99' or bossMungFlag[i] == True :
					if bossMungFlag[i] == True :
						if bossData[i][2] == '0' :
							information1 += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + tmp_bossTime[i].strftime('%H:%M:%S') + ' @ ' + tmp_bossTime[i].strftime('%Y-%m-%d') + ' (нет ввода ' + str(bossMungCnt[i]) + ')' + ' * ' + bossData[i][6] + '\n'
						else : 
							information1 += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + tmp_bossTime[i].strftime('%H:%M:%S') + ' @ ' + tmp_bossTime[i].strftime('%Y-%m-%d') + ' (멍 ' + str(bossMungCnt[i]) + ')' + ' * ' + bossData[i][6] + '\n'
					else:
						if bossData[i][2] == '0' :
							information1 += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + bossTimeString[i] + ' @ ' + bossDateString[i] + ' (нет ввода ' + str(bossMungCnt[i]) + ')' + ' * ' + bossData[i][6] + '\n'
						else : 
							information1 += ' - ' + bossData[i][0] + '(' + bossData[i][1] + '.' + bossData[i][5] + ') : ' + bossTimeString[i] + ' @ ' + bossDateString[i] + ' (멍 ' + str(bossMungCnt[i]) + ')' + ' * ' + bossData[i][6] + '\n'
							...
							if basicSetting[2] != '0' and basicSetting[22] != '0' and bossFlag[i] == True and bossFlag0[i] == True and bossMungFlag[i] == True :
								if int(basicSetting[17]) <= bossMungCnt[i] and int(basicSetting[17]) != 0:
									...
									if bossData[i][2] == '0':
										...
										embed = discord.Embed(
											description= '```Следующий ' + bossData[i][0] + ' ' + bossTimeString[i] + '입니다.```',
											color=0xff0000
											)
										await self.bot.get_channel(channel).send(embed=embed, tts=False)
										try:
...
class mainCog(commands.Cog): 
	...
	async def menu_(self, ctx):
		if ctx.message.channel.id == basicSetting[7]:
			...
			command_list += '[имя босса] удалить\n'     
			command_list += '[имя босса]메모 [할말]\n'
			embed = discord.Embed(
					title = "----- 명령어 -----",
					...
					for j in range(int(basicSetting[9])):
						for i in range(len(ouput_bossData)):
							if sorted_datelist[j] == ouput_bossData[i][1]:
								...
								result_lefttime += 'Следующий ' + ouput_bossData[i][0] + '탐까지 %02d:%02d:%02d 남았습니다. ' % (hours,minutes,seconds) + '[' +  ouput_bossData[i][2] + ']\n'
				else :
					...
					if message.content.startswith(bossData[i][0] +'멍') or message.content.startswith(bossData[i][0] +' 멍'):
						...
						if len(hello) > len(tmp_msg) + 3 :
							...
							if tmp_bossTime[i] < tmp_now :
								...
								await self.get_channel(channel).send('```' + bossData[i][0] + '탐이 아직 안됐습니다. Следующий ' + bossData[i][0] + '탐 [' + tmp_bossTimeString[i] + '] 입니다```', tts=False)
